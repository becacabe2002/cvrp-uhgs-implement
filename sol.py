...
class solution:
    def __init__(self, grantTour, p):
        # p stands for population
        self.cost = -1
        self.feas = True
        self.GT = grantTour
        self.penality = p.omega
        _, self.delimiters = split(grantTour, p, self.penality)
    ...
    def M1(self, p, c1, c1i, r1i, c2, c2i, r2i):  # CROSS 1-0
        ...
        if r1i == r2i:
            ...
            if newcost < self.sigma[r1i].cost:
                self.sigma[r1i].cost = newcost
        ...
        if r1newcost + r2newcost < self.sigma[r1i].cost + self.sigma[r2i].cost:
            self.sigma[r1i].cost = r1newcost
            self.sigma[r2i].cost = r2newcost
...
class Route:
    ...
    def recomputecost(self, p, penality):
        try:
            ...
            self.cost = self.subseq[0][-1].cost
            self.capacity = self.subseq[0][-1].capacity
            self.feas = True
            loadexcess = max(0, self.capacity - p.C)
            if loadexcess > 0:
                self.feas = False
                self.cost += loadexcess * penality
            return self.cost
        except:
            self.cost = 0
            self.feas = True
            ...
            for i in range(mat_dim):
                ...
                for j in range(i):
                    self.subseq[i].append(seqdata(
                        self.subseq[j][i].cost, self.subseq[j][i].capacity, self.subseq[j][i].end,
                        self.subseq[j][i].start))
                for j in range(i, mat_dim):
                    sub_capacity += p.demand[self.customer[j]]
                    if i != j:
                        sub_cost += p.dist[self.customer[j - 1]
                        ][self.customer[j]]
                    self.subseq[i].append(
                        seqdata(sub_cost, sub_capacity, self.customer[i], self.customer[j]))


class seqdata:
    def __init__(self, cost, capacity, start, end):
        self.cost = cost
...
def routing(grantTour, delimiters, p, penality):
    sigma = []
    tour = grantTour[:]
    route = []
...
def split(grantTour, p, penality):
    route = grantTour[:]
    route.insert(0, 0)
    edges = []
    for i in range(len(route)):
        ...
        for j in range(i + 1, len(route)):
            ...
            sigma = seqdata[:]
            sigma.append(0)
            feasible, cost = evalutation.cost(sigma, p, penality)
            if feasible:
                edges.append((i, j, cost))
            else:
                break
    cost, delimiters = utils.shortestPath(edges, 0, len(route) - 1)
    return cost, delimiters
...
def evalN(p, seq, penality):
    ...
    for i in range(seq_num):
        capacity += seq[i].capacity
        cost += seq[i].cost
        cost += p.dist[seq[i].end][seq[i + 1].start]
    cost += seq[seq_num].cost
    capacity += seq[seq_num].capacity
    loadexcess = max(0, capacity - p.C)
    cost += loadexcess * penality
    return cost
